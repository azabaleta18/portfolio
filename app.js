...
(() => {
  ...
  // Desktop vs mobile: on mobile use native scroll (no LERP fighting touch)
  ...
  // setBodyHeight is no-op on mobile; assigned below on desktop
  let setBodyHeight = () => {};
  ...
  if (!isMobile) {
    ...
    function raf() {
      ...
      const settled = Math.abs(targetScroll - currentScroll) < 0.05;
      ...
      // Stop RAF when settled — no idle 60fps burn
      rafId = settled ? null : requestAnimationFrame(raf);
    }
    ...
    rafId = requestAnimationFrame(raf); // initial frame

  } else {
    // Mobile: disable smooth wrapper, let native scroll handle everything
  ...
  document.querySelectorAll('a[href^="#"]').forEach(a => {
    a.addEventListener('click', e => {
      ...
      if (!target) return;
      e.preventDefault();
      const baseScroll = isMobile ? window.scrollY : currentScroll;
      window.scrollTo({ top: target.getBoundingClientRect().top + baseScroll - 90, behavior: 'smooth' });
    });
  });
  ...
  function buildVisual(p, title) {
    ...
    if (p.img) {
      parts.push(`<img class="proj-cover" src="${esc(p.img)}" alt="${esc(title)}" loading="lazy" decoding="async">`);
    }
    const gallery = asArray(p.gallery);
    if (gallery.length) {
      parts.push(`<div class="proj-gallery">${
        gallery.map(url => `<img src="${esc(url)}" alt="${esc(title)}" loading="lazy" decoding="async">`).join('')
      }</div>`);
    }
    if (!parts.length) {
